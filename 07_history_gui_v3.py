...
class HistoryExport:
    ...
    def __init__(self, partner, calc_list):
        ...
        self.filename_feedback_label.grid(row=6)
    ...
    def make_file(self):
        ...
        if filename_ok == "":
            filename += ".txt"
            success = f"Success! Your calculation history has been saved as {filename}"
            self.var_filename.set(filename)
            self.filename_feedback_label.config(text=success, fg="dark green")
            
            self.filename_entry.config(bg="#FFF")
    ...
    def write_to_file(self):
        ...
        generated = f"Generate0d: {generated_date}\n"
